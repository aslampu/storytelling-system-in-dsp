	...
	while(1) {
		if(flag) {
			...
			for (i=0;i<320;i+=2) {  // Conversion yuv to RGB565
				...
				rTemp0 = ((float)(rgbOri[0]&0xF800))/31;
				gTemp0 = ((float)(rgbOri[0]&0x7E0))/31;
				bTemp0 = ((float)(rgbOri[0]&0x1F))/31;
	
				rTemp0 = ((float)(rgbOri[1]&0xF800))/31;
				gTemp0 = ((float)(rgbOri[1]&0x7E0))/31;
				bTemp0 = ((float)(rgbOri[1]&0x1F))/31;
