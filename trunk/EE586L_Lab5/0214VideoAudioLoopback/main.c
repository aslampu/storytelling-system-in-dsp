...
unsigned short maskFactory(Uint32 (*dip_value)(Uint32), void (*led_on)(Uint32), void (*led_off)(Uint32)){
	enum colorID {ALL_Color, R_Color, G_Color, B_Color};
	switch(!dip_value(1)*2+!dip_value(2)){
		case ALL_Color:
			mask=0xffff;
			led_off(1);
			led_off(2);
...
void displayFactory(Display *state, Uint32 (*dip_value)(Uint32), void (*led_on)(Uint32), void (*led_off)(Uint32)){
	//stepIn = !DSK6713_DIP_get(3);
	state->stepIn = dip_value(3);
...
void main()
{
	...
	//function pointers
	void (*led_on)(Uint32) = 0;
	void (*led_off)(Uint32) = 0;
	Uint32 (*dip_value)(Uint32) = 0;
	...
	led_on = DSK6713_LED_on;
	led_off = DSK6713_LED_off;
	dip_value = DSK6713_DIP_get;

	intr_enable(6);
	intr_enable(15);

	while(1) {
		//Modified: stepIn, move, drawBox
		//displayFactory(&state, dip_value, led_on, led_off);
		...
		mask = maskFactory(dip_value, led_on, led_off);	
