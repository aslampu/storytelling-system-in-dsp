...
void RGB2Lab(unsigned short rgbColor, short *ptr_LValue, short *ptr_aValue, short *ptr_bValue){
	...
	X /= (255 * 0.950456);
  	...
  	if (Y > 0.008856){
    	fY = pow(Y, 1.0 / 3.0);
      *ptr_LValue = (int)(116.0 * fY - 16.0 + 0.5);
    }else{
      fY = 7.787 * Y + 16.0 / 116.0;
      *ptr_LValue = (int)(903.3 * Y + 0.5);
    }
  	...
  	*ptr_aValue = (int)(500.0 * (fX - fY) + 0.5);
  	*ptr_bValue = (int)(200.0 * (fY - fZ) + 0.5);
	//-128~127
...
unsigned short Lab2RGB(short L, short a, short b){	
	//int L;
	//int a;
	//int b;
	float X, Y, Z, fX, fY, fZ;
